...
import { getRefValue, getTouchEventData } from '../Slider.helper';

import { TSlider } from './types';

const MIN_SWIPE_REQUIRED = 40;

const Slider = ({ data, dots }: TSlider) => {
  ...
  return (
    <div className={styles.slider_container} onTouchStart={onTouchStart} onMouseDown={onTouchStart}>
      <div
        ref={containerRef}
        className={classNames(styles.slider_list, {
          [styles.slider_list_is_swiping]: isSwiping,
        })}
        style={{ transform: `translate3d(${offsetX}px, 0, 0)` }}
      >
        {data.map((item, idx) => (
          <SliderContent key={idx} {...item} />
        ))}
      </div>
      {dots && <Dots activeIndex={currentIdx} data={data} onclick={indicatorOnClick} />}
    </div>
  );
};
